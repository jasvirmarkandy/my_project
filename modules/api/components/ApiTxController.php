...
abstract class ApiTxController extends ActiveController
{
    ...
    public function txDeleteRank($id,$mname=null)
    {
        if($mname ==null || empty($mname)){
            
            $model = $this->findModelforRank($id,'StyleRank');
            $data['status'] = self::API_NOK;
            if ($model->delete()) {
                $data['status'] = self::API_OK;
                $data['msg'] = $this->modelClass . ' is deleted Successfully.';
            ...
            $model = $this->findModelforRank($id,$mname);
            $data['status'] = self::API_NOK;
            if ($model->delete()) {
                $data['status'] = self::API_OK;
                $data['msg'] = $this->modelClass . ' is deleted Successfully.';
    ...
    public function txDeleteProduct($id)
    {
        $model = $this->findModelforRank($id,'StyleRank');
        $data['status'] = self::API_NOK;
        if ($model->delete()) {
    ...
    protected function findModelforRank($id,$model)
    {
        $modelClass = Inflector::id2camel(\Yii::$app->controller->id);
